		...
		// event handling
			...
			if ( SDL_PollEvent(&e) ) {
				...
				if (e.type == SDL_QUIT)
					...
					break;
				//
				// set on down and del on release
				...
				//pw+=10;
				//ph+=int(e.tfinger.x);
					//ph=int(e.tfinger.y*HEIGHT);
					//pw=int(e.tfinger.x*WIDTH);
			...
			if (ii>10){
				...
				if ((gxx<0 or gxx>WIDTH) and  (gyy<0 or gyy>HEIGHT)){
				    gfx0*=-1;
				    gfy0*=-1;
				    while ((gxx<0 or gxx>WIDTH) and  (gyy<0 or gyy>HEIGHT) ){
				        gxx+=gfx0;
				        gyy+=gfy0;
				    }	
				}
				if (gxx<0 or gxx>WIDTH){
				    gfx0*=-1;
				    if (gxx<0 or gxx>WIDTH){
				        gxx+=gfx0;
				    }
				}
				if (gyy<0 or gyy>HEIGHT){
				    gfy0*=-1;
				    while(gyy<0 or gyy>HEIGHT){
				        gyy+=gfy0;
				    }
				        
				}
				if ((dxx<0 or dxx>WIDTH) and (dyy<0 or dyy>HEIGHT) ){
				    dfx0*=-1;
				    dfy0*=-1;
				    while ((dxx<0 or dxx>WIDTH) and (dyy<0 or dyy>HEIGHT) ){
				        dxx+=dfx0;
				        dyy+=dfy0;
				    }	
				}
				if (dxx<0 or dxx>WIDTH){
				    dfx0*=-1;
				    if (dxx<0 or dxx>WIDTH){
				        dxx+=dfx0;
				    }
				}
				if (dyy<0 or dyy>HEIGHT){
				    dfy0*=-1;
				    while(dyy<0 or dyy>HEIGHT){
				        dyy+=dfy0;
				    }
